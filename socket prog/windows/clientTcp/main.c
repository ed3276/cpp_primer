...
int main()
{
    ...
    int f = connect(s,(struct sockaddr*)&servAddr,sizeof(servAddr));
    if(f) {printf("connect is Failure. error code: %d.\n",WSAGetLastError());printf("ERROR INFO: %s\n",strerror(errno));perror("FAIL info:");exit(1);}
    ...
    buffer[len]='\0';
    printf("Echo string received %u characters: %s\n\n",strlen(buffer),buffer);
    ...
    for(int i=0;i<size;++i)
    {
        ...
        len = recv(s,buffer,sizeof(buffer),0);
        buffer[len]='\0';
        printf("Echo string received %uLL characters: \n%s\n\n",strlen(buffer),buffer);
        //Sleep(10);
    }
    closesocket(s);
